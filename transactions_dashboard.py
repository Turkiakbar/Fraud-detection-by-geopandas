...
with tab5:
    ...
    if 'gender' in filtered_df.columns:
        ...
        fig = px.pie(counts, names='gender', values='count', title='Gender Distribution', hole=0.3)
    ...
    if 'state' in filtered_df.columns:
        ...
        fig = px.bar(sc, x='state', y='count', title=f'Fraud Count by State (Top {top_n})')
    ...
    if 'age' in filtered_df.columns:
        ...
        fig = px.bar(ac, x='age_group', y='count', title='Transactions by Age Group')
        st.plotly_chart(fig, use_container_width=True)

    # Category counts
    if 'category' in filtered_df.columns:
        cc = filtered_df['category'].value_counts().rename_axis('category').reset_index(name='count')
        fig = px.bar(cc, x='category', y='count', title='Fraud Count by Category')
        fig.update_layout(xaxis_tickangle=-45)
        st.plotly_chart(fig, use_container_width=True)
    ...
    if 'amt' in filtered_df.columns:
        ...
        fig = px.bar(gc, x='amount_range', y='count', title='Transactions by Amount Range')
        st.plotly_chart(fig, use_container_width=True)

    # Fraud by hour and weekday
    if 'trans_datetime' in filtered_df.columns:
        # Hour with bins from min to max hour present in data
        ...
        fig = px.bar(hc, x='hour', y='count', title=f'Fraud Count by Hour of Day ({min_hour}:00 to {max_hour}:00)')
        st.plotly_chart(fig, use_container_width=True)
        ...
        fig = px.bar(wc, x='weekday', y='count', title='Fraud Count by Weekday')
        st.plotly_chart(fig, use_container_width=True)


#python -m streamlit run "D:\Cursor_projects\Tuwaiq_Bootcamp\Project#3\transactions_dashboard.py"